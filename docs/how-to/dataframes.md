...
[`ForecastModel.forecast_df`] forecasts a
dataframe directly, with no need to build [`TimeseriesType`]
...
a target. Add `multivariate=True` to forecast them **jointly**, so the model can use the
...
of series is one call rather than a Python loop. On a CUDA or MPS out-of-memory error the batch
size is halved and the failing batch retried.
