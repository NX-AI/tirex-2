...
def test_univariate_mean(api_server):
    ...
    assert_forecast_close(data, REFERENCE.uni[:, MEDIAN_QUANTILE_INDEX])
...
def test_univariate_quantiles(api_server):
    ...
    assert_forecast_close(data, REFERENCE.uni)
...
def test_multivariate_mean(api_server):
    ...
    assert_forecast_close(data, [REFERENCE.multi[:, MEDIAN_QUANTILE_INDEX, :]])
...
def test_multivariate_quantiles(api_server):
    ...
    assert_forecast_close(data, [REFERENCE.multi])
...
def test_univariate_batch(api_server):
    ...
    assert_forecast_close(data, [REFERENCE.uni[0, MEDIAN_QUANTILE_INDEX], REFERENCE.uni[0, MEDIAN_QUANTILE_INDEX]])
...
def test_multivariate_batch(api_server):
    ...
    assert_forecast_close(data[0], REFERENCE.multi[:, MEDIAN_QUANTILE_INDEX, :])
